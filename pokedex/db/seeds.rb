...
def create_moves(moves, pokemon)
  ...
  moves.each do |m|
    ...
    obj_moves.push(move) if move
  end
  pokemon.moves << obj_moves
end

def create_sprites(sprites, pokemon)
  sprites_names = %i[back_default back_female back_shiny back_shiny_female
...
def create_stats(stats, pokemon)
  stats.each do |s|
    ...
    join = PokemonsStat.new(base_stat: s['base_stat'], effort: s['effort'], pokemon: pokemon,
                            stat: stat)
    join.save
  end
end
...
def seed_type(type_class, url, params = { name: 'name', url: 'url' })
  ...
  records.each do |a|
    db_params = params.transform_values { |v| a[v] }
...
def seed_maestros
  ...
  pokemons = []
  names = %w[pikachu bulbasaur snorlax]
